...
***
Немного об итоговом проекте по домашнему заданию. Этот проект является гибридным, кроме основных заданий, в составе проекта включены задания с *.  Из задания со * в проекте:
- module vpc и структура tfvars интегрирована и переработан с учетом задания 4
- модуль (mdb_mysql_cluster) для создания кластера managed БД Mysql, также интегрирован с учетом задания 5. Управление режимами отключение и включение данного модуля осуществляется через variable "cluster_create", по умолчанию включено (true). 
- Дополнительно добавлено условие, согласно заданию 5, управления режимом HA (high availability).
  HA=false применение одной VM, HA=true применение высокой доступности из 2 VM, управляется variable  «high_availability» в основном блоке настройки кластера «conf_root_cl» в файле tfvars. По умолчанию режим HA=true

> !!! Для тех, кто захочет протестировать данный репозиторий, следите за денежным балансом в Yandex cloud. Применение платформы данных Managed Service for MySQL может динамично опустошить баланс на ноль. В проекте указаны минимальные параметры платформы и класса хоста, потребление составляет чуть больше 4000 рублей в месяц !!!

В проекте по мимо основных файлов terraform есть три файла passdb.key, passwd.gen и passwd.key
- passwd.gen – в этом файле указывается пароль для последующей генерации его в HASH и добавление в файл passwd.key. Генерируется в блоке resource "null_resource" "gen_passwd" в файле cloud-conf.tf, только один раз, так как cloud init выполняется при самом первом запуске VM. 
- passwd.key – готовый сгенерированный HASH пароль для передачи его в cloud init через metadata.
- passdb.key – указывается пароль к DB, в случае применения кластера MySQL, с дальнейшим его передачи модулю mysql. 
***
